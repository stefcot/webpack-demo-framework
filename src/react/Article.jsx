...
class Article extends React.Component {
    ...
    render() {
        const { type, image, index, title, author, contentPath, isPremium, lead, related } = this.props;

        console.log('Article::render - related: ', related);

        return (
            <article className={`${styles['root']} ${styles[type]}`} id={`art-cmp-${type}-${index}`}>
                <h2>{htmlToReactParser.parse(title)}</h2>
                {typeof image !== 'undefined' && image.imgPath !== '' && (
                    <Figure contentPath={contentPath} index={index} isPremium={isPremium} {...image} />
                )}
                {typeof author !== 'undefined' && author !== '' && (
                    <address>par {htmlToReactParser.parse(author)}</address>
                )}
                <p>{htmlToReactParser.parse(lead)}</p>
                {typeof related !== 'undefined' && related.length > 0 && <Related list={related} />}
            </article>
        );
    }
}
